# -*- coding: utf-8 -*-
"""
Pipeline Fases 3 a 5: Modelagem Preditiva Temporal (Desde 2020)
Treina e compara os 5 classificadores e Stacking Ensemble usando apenas dados de 2020 em diante.
"""
...
def mcfadden_pseudo_r2(y_true, y_prob):
    ...
    # Log-likelihood do modelo nulo (apenas intercepto)
...
def find_best_threshold(y_true, y_prob):
    """Encontra o limiar que maximiza o F1-Score da classe positiva"""
...
def main():
    ...
    features_file = os.path.join(project_root, "dataset", "features_selecionadas_binaria.json")
    output_dir = os.path.join(project_root, "dataset")
    
    print("Carregando base limpa e lista de features selecionadas...")
    ...
    # -------------------------------------------------------------
    # Filtragem Temporal (Desde 2020)
    # -------------------------------------------------------------
    print("\n--- Aplicando Filtro Temporal: Acidentes desde 2020 ---")
    df['data_inversa'] = pd.to_datetime(df['data_inversa'])
    df_filtered = df[df['data_inversa'] >= '2020-01-01'].copy()
    print(f"Dataset filtrado (desde 2020). Shape: {df_filtered.shape}")
    
    # Descarte das colunas de controle temporal e espacial bruto
    df_filtered = df_filtered.drop(columns=['data_inversa', 'pais'])
    
    # -------------------------------------------------------------
    # Codificação e Divisão Treino/Teste
    # -------------------------------------------------------------
    ...
    joblib.dump(scaler, os.path.join(output_dir, "scaler_desde2020.joblib"))
    
    base_models = {}
    
    # -------------------------------------------------------------
    # 1. Regressão Logística (unweighted para manter calibração)
    # -------------------------------------------------------------
    ...
    # -------------------------------------------------------------
    # 3. Random Forest (Bagging otimizado com max_samples=0.10)
    # -------------------------------------------------------------
    ...
    if TENSORFLOW_AVAILABLE:
        # MLP no Keras com Batching rápido
        mlp_model = build_keras_mlp(X_base.shape[1])
        
        # Calcula pesos de amostra
    ...
    joblib.dump(logit, os.path.join(output_dir, "model_logit_desde2020.joblib"))
    joblib.dump(lda, os.path.join(output_dir, "model_lda_desde2020.joblib"))
    joblib.dump(rf, os.path.join(output_dir, "model_rf_desde2020.joblib"))
    joblib.dump(xgb, os.path.join(output_dir, "model_xgb_desde2020.joblib"))
    joblib.dump(meta_learner, os.path.join(output_dir, "model_stacking_desde2020.joblib"))
    if not TENSORFLOW_AVAILABLE:
        joblib.dump(mlp_model, os.path.join(output_dir, "model_mlp_desde2020.joblib"))
    else:
        mlp_model.save(os.path.join(output_dir, "model_mlp_desde2020.h5"))
        
    # Coeficientes do Stacking
    print("\nPesos das Contribuições no Stacking Ensemble (desde 2020):")
    ...
    with open(os.path.join(output_dir, "limiares_decisao_desde2020.json"), 'w') as f:
    ...
    for name in all_eval_probs.columns:
        ...
        plt.title(f"Matriz Confusão (Limiar {opt_thresh:.2f}) - {name} (desde 2020)", fontsize=11, color='#00f0ff', pad=15)
        ...
        plt.savefig(os.path.join(output_dir, f"matriz_confusao_{safe_name}_desde2020.png"), dpi=300, facecolor='#121214')
        plt.close()
        
    metrics_df = pd.DataFrame(metrics_summary)
    print("\nTabela Comparativa de Métricas (Stacking e Threshold Tuning - desde 2020):")
    print(metrics_df.to_string(index=False))
    metrics_df.to_csv(os.path.join(output_dir, "comparativo_modelos_desde2020.csv"), index=False)
    ...
    plt.title('Comparativo de Modelos Preditivos (Desde 2020)', fontsize=14, color='#00f0ff', pad=15)
    ...
    plt.savefig(os.path.join(output_dir, "comparativo_performance_desde2020.png"), dpi=300, facecolor='#121214')
    plt.close()
    ...
    plt.title("Importância das Variáveis (Top 15 - XGBoost desde 2020)", fontsize=14, color='#00f0ff', pad=15)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "importancia_features_xgboost_desde2020.png"), dpi=300, facecolor='#121214')
    plt.close()
    
    print("\nSorted Feature Importances (XGBoost desde 2020):")
