...
def plot_smote_distribution(y_before, y_after, output_dir):
    """
    Gera um gráfico de barras agrupadas comparando as distribuições de classes antes e depois do SMOTE.
    """
    apply_chart_style()
    print("Gerando gráfico comparativo do balanceamento SMOTE...")
    ...
    counts_after = y_a.value_counts().sort_index()
    ...
    # Rosa neon para antes, ciano neon para depois
    rects1 = ax.bar(x - width/2, counts_before.values, width, label='Antes do SMOTE (Desbalanceado)', color='#ff007f')
    rects2 = ax.bar(x + width/2, counts_after.values, width, label='Depois do SMOTE (Balanceado)', color='#00f0ff')
    
    ax.set_title('Impacto do Balanceamento SMOTE nas Classes de Severidade', fontsize=14, color='#00f0ff', pad=15)
    ax.set_xlabel('Classes de Severidade', fontsize=12, color='#e2e2e9')
    ax.set_ylabel('Quantidade de Registros', fontsize=12, color='#e2e2e9')
    ax.set_xticks(x)
    ...
    plt.close()
    print(f"Gráfico de distribuição SMOTE salvo em: {out_path}")
...
def run_pipeline():
    ...
    # Para garantir viabilidade computacional em máquinas locais com bases massivas (como 7.5M de linhas),
    # aplicamos uma subamostragem estratificada controlada se o dataset for superior a 350k linhas.
    if len(df) > 350000:
        print(f"Dataset massivo detectado ({len(df):,} registros). Extraindo amostra estratificada controlada de 350.000 registros para viabilizar SMOTE/XGBoost localmente...")
        df, _ = train_test_split(df, train_size=350000, stratify=df['Severidade'], random_state=42)
        print(f"Subamostragem concluída. Novo tamanho: {len(df):,}")
    ...
    # Gera o gráfico de comparação das classes pré e pós SMOTE
    ...
    xgb_metrics = evaluate_model("XGBoost (Tuned)", y_test, y_pred_xgb, y_prob_xgb, c_max, c_prop)
    metrics_dict["XGBoost (Tuned)"] = xgb_metrics
    ...
    lr_model = LogisticRegression(max_iter=200, solver='lbfgs', random_state=42, n_jobs=-1)
